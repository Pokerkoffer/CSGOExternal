...
namespace Overlay
{
	...
	void CD3D9Surface::String ( int x, int y, const std::string &strSurfaceFontName, Color color, const char *pszString, ... )
	{
		...
		RECT rect_angle = this->CalculateString (strSurfaceFontName, color, pszString);
		...
		( ( LPD3DXFONT ) pSurfaceFontData->m_pSurfaceFont )->DrawTextA ( nullptr, pszBuffer, -1, &rect_angle, DT_TOP | DT_LEFT, color.GetHex ( ) );
	}

	RECT CD3D9Surface::CalculateString (const std::string & strSurfaceFontName, Color color, const char * pszString, ...)
	{
		auto pSurfaceFontData = this->GetSurfaceFontData (strSurfaceFontName);
		if (!pSurfaceFontData)
			return { 0, 0, 0, 0 };
		...
		RECT calc_rect_angle = { 0, 0, 0, 0 };

		((LPD3DXFONT)pSurfaceFontData->m_pSurfaceFont)->DrawTextA (nullptr, pszBuffer, -1, &calc_rect_angle, DT_CALCRECT, color.GetHex ());

		return calc_rect_angle;
	}
}